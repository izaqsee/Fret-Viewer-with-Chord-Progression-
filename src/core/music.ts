/**
 * 音名や度数の計算など、アプリ全体で使い回す音楽理論のユーティリティを集約したモジュール。
 *
 * - pcOf / pcName / degreeLabel など、音階と調号に依存する処理をコンポーネントから切り離してテストしやすくする。
 * - buildNotes ではチューニングとフレット範囲からフレットボード上の座標を生成し、FretboardSVG へ渡すデータを作る。
 * - App.tsx 以外のファイルからも再利用する前提で、状態を持たない純粋関数として提供している。
 */
